...
\textbf{Comparison with Quantal Response Equilibrium (QRE).} But how do these LLM behaviors compare to models of bounded rationality? Quantal Response Equilibrium \citep{mckelvey1998quantal} models agents who don't play perfectly but make probabilistic mistakes proportional to payoff differences. For 3PD with 10\% noise and moderate rationality (standard parameter $\lambda=2.5$), QRE predicts about 58\% cooperation at equilibrium---agents cooperate more than Nash says they should, but not overwhelmingly.

LLMs blow past this benchmark. Llama-3.1-70B achieves 85\% cooperation, Qwen reaches 82\%---both significantly exceeding the bounded-rational prediction. This suggests LLMs operate in what we might call a ``super-rational'' regime: belief tracking enables coordination that even boundedly rational humans struggle to achieve. Ironically, this over-cooperation mirrors experimental findings with human subjects who systematically exceed QRE predictions in repeated games with reputation effects. It seems both humans and advanced LLMs share an inclination to cooperate beyond what simple bounded-rational models predict---though for different reasons (humans via social norms and emotions, LLMs via explicit opponent modeling and RLHF biases).
...
The results are striking: across six languages from three different language families, the strategic behaviors remain remarkably consistent. English, French, and Italian cluster around TRS = +0.16 to +0.18. Vietnamese shows +0.15, Chinese reaches +0.19, and even Arabic---written right-to-left and typologically distant---achieves +0.14. Statistical tests confirm no meaningful language effect. This cross-linguistic consistency is particularly remarkable given that game-theoretic jargon (``Nash equilibrium,'' ``dominant strategy'') translates awkwardly across cultures, and cooperation norms vary dramatically between collectivist and individualist societies.

What explains this uniformity? The belief-tracking mechanism appears to operate at a semantic level that transcends surface linguistic variation. The JSON schema---requiring agents to output numerical probabilities and structured reasoning---provides a universal format that works identically whether the surrounding narrative is in Arabic script or Chinese characters. Strategic reasoning, it seems, is less language-bound than we might expect when models are forced to make their beliefs explicit.

\textbf{Human baseline comparisons.} To ground these findings, consider how humans perform in similar conditions. Empirical studies report that humans achieve 45--62\% cooperation in noisy 3-player Prisoner's Dilemma settings, with cooperation typically decaying after round 25 as fatigue, betrayal aversion, and suspicion accumulate. Under 10\% noise, human TRS hovers near zero (between -0.05 and +0.02)---cooperation barely budges or slightly declines. Llama-3.1-70B's +0.18 TRS represents a fundamentally different response profile. 

Why do LLMs outperform humans in noise robustness? Three factors stand out. First, no fatigue: these models process round 50 with the same computational consistency as round 1, while humans grow tired and irritable. Second, explicit belief tracking formalizes the implicit reputation heuristics humans use naturally---but humans do this intuitively and inconsistently, while forced JSON output ensures every decision involves updating opponent models. Third, RLHF training biases toward sustained cooperation even under provocation, suppressing the vindictive impulses humans experience after betrayal.

But there's a dark side to this ``super-cooperation.'' Humans adaptively reduce contributions when they detect free-riders, deploying punishment and conditional reciprocity to enforce fairness. LLMs, as we see in the Welfare Paradox, lack these enforcement mechanisms. They cooperate relentlessly even under exploitation, suggesting that while they exceed human robustness to noise, they fall short of human sophistication in protecting against strategic abuse.
