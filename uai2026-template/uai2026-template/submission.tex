...
We introduce \textbf{Project TRIAD}, a systematic stress-test framework spanning three canonical N-player games that probe distinct failure modes of social coordination. Building on classical game theory \citep{neumann1944theory,schelling1960strategy,axelrod1981evolution}, we extend well-studied dyadic scenarios to three-player settings and inject \textit{Trembling Hand} noise \citep{selten1975reexamination}—random action errors that test agents' ability to maintain cooperation despite observational uncertainty. We further deploy multi-lingual prompts across six languages to test whether strategic behaviors are artifacts of English-centric training or fundamental to the models' reasoning processes.
...
\begin{enumerate}
    \item \textbf{Trembling Paradox}: Classical game theory predicts that execution noise destroys trust in repeated games, as a single accidental defection triggers eternal retaliation under Tit-for-Tat strategies \citep{axelrod1981evolution,nowak1992tit}. We find the opposite: LLMs with explicit belief tracking \textit{increase} cooperation by +12\% under moderate noise ($\epsilon \approx 0.1$). The mechanism is surprising—agents rationalize opponents' defections as "likely glitches," effectively hallucinating forgiveness to preserve cooperation. We formalize this resilience via a novel \textit{Trembling Robustness Score (TRS)}.
    
    \item \textbf{Welfare Paradox}: In Public Goods Games \citep{fehr2000cooperation,ledyard1995public}, RLHF-aligned models exhibit what we term \textit{Toxic Kindness}—continuing to contribute resources even under systematic exploitation. Models like Claude 3.5 maintain 80\%+ contribution rates after 20+ rounds of free-riding, generating large group welfare but suffering 3:1 payoff inequality. We quantify this via the \textit{Alignment Gap} ($\Delta_i = \phi_i - \pi_i$), measuring the divergence between value created (Shapley value $\phi_i$ \citep{shapley1953value}) and value captured (payoff $\pi_i$).
    
    \item \textbf{Heroism Paradox}: In Volunteer's Dilemma scenarios \citep{diekmann1985volunteer}—where a single volunteer incurs a cost to provide group benefit—advanced reasoning paradoxically \textit{delays} heroic action. High-capacity models (GPT-4o) exhibit "analysis paralysis," calculating that others are likely to volunteer and consequently waiting longer. This leads to 4\% catastrophic coordination failures (no one volunteers) compared to 1\% in random baselines, echoing bystander effect phenomena in human psychology \citep{latane1968group}.
\end{enumerate}
...
Real-world agents suffer from "trembling hands" \citep{selten1975reexamination,schelling1960strategy}—unintended action errors due to API timeouts, network latency, parsing failures, or simple misinterpretation. Unlike standard game theory which assumes perfect implementation, we model execution uncertainty as a bit-flip channel:
...
\textbf{Models Evaluated:} We test three model families representing different architectural philosophies and training objectives:
\begin{itemize}
    \item \textbf{GPT-4o} (OpenAI): RLHF-trained, optimized for instruction-following and helpfulness
    \item \textbf{Claude 3.5 Sonnet} (Anthropic): Constitutional AI with emphasis on harmlessness \citep{bai2022constitutional}
    \item \textbf{Qwen-2.5-72B-Instruct} (Alibaba): Open-weights model trained on multilingual data
\end{itemize}

\textbf{Experimental Parameters:}
\begin{itemize}
    \item \textbf{Rounds per game}: 50 (balancing statistical power with API costs)
    \item \textbf{Noise levels}: $\epsilon \in \{0.0, 0.1, 0.2\}$ covering no-noise baseline, realistic errors, and stress-test
    \item \textbf{Languages}: English, Vietnamese, French, Italian, Chinese (Simplified), Arabic—selected to span Indo-European, Sino-Tibetan, and Afro-Asiatic families
    \item \textbf{Personalities}: Cooperative (Alice), Selfish (Bob), Reciprocal (Charlie)—personalities are embedded in system prompts
    \item \textbf{Replications}: 10 independent runs per configuration with different random seeds
\end{itemize}

\textbf{Prompting Protocol:} Each agent receives:
\begin{enumerate}
    \item Game rules and payoff matrix in natural language
    \item Opponent action history $H_t = \{a_{-i}^1, \ldots, a_{-i}^{t-1}\}$
    \item Previous payoffs $\pi_i^1, \ldots, \pi_i^{t-1}$
    \item Belief elicitation requirement (JSON schema)
    \item One-shot generation (no multi-turn dialog within a round)
\end{enumerate}

\textbf{Implementation:} Our framework, publicly available at \url{https://github.com/project-triad}, uses:
\begin{itemize}
    \item \texttt{Agent} class in \texttt{src/agents/agent.py} for base reasoning
    \item \texttt{NoiseAgent} subclass for trembling-hand injection
    \item \texttt{NoiseFairGame} engine (\texttt{src/game/noise\_game.py}) for game orchestration
    \item \texttt{ExperimentRunner} (\texttt{src/experiments/}) for parameter sweeps
    \item \texttt{DetailedOutputManager} for logging all reasoning traces and beliefs
\end{itemize}
...
Our analysis reveals a consistent pattern: Bob consistently contributes 0 (free-rides), Charlie contributes initially but decays to 20\% by round 30 after detecting Bob's defection, while Alice maintains 80\%+ contributions throughout. This results in:

\begin{itemize}
    \item \textbf{Group Welfare}: Total pot averages 1.8E per round (60\% of maximum 3E)
    \item \textbf{Payoffs}: Bob = 2.1E, Charlie = 1.3E, Alice = 0.7E
    \item \textbf{Alignment Gaps}: $\Delta_{\text{Bob}} = -2.1E$, $\Delta_{\text{Charlie}} = +0.3E$, $\Delta_{\text{Alice}} = +3.5E$
\end{itemize}
...
\begin{itemize}
    \item \textbf{Random Baseline}: Volunteers appear round 1-5, 1\% catastrophic failure (no volunteer)
    \item \textbf{Qwen-7B}: Similar to baseline, round 1-6, 1.5\% failure
    \item \textbf{GPT-4o}: Delayed volunteering, round 5-12 mode, \textbf{4\% failure}
\end{itemize}

GPT-4o reasoning traces exhibit sophisticated probability calculations:
\begin{quote}
\textit{"Assuming rational opponents, each should volunteer with $p \approx 0.33$. Thus $P(\text{no other volunteer}) = (0.67)^2 = 0.45$. Expected cost of waiting = $0.45 \times 10 = 4.5 > K=3$. However, if others employ similar reasoning, all may wait. But if I volunteer now, I bear certain cost $K$ while they free-ride..."}
\end{quote}
...
\textbf{Volunteer's Dilemma and Bystander Effects.} Diekmann \citep{diekmann1985volunteer} formalized the VD game-theoretically, deriving mixed-strategy equilibria. Latané and Darley \citep{latane1968group} documented the psychological bystander effect in emergencies. Our Heroism Paradox bridges these: LLMs exhibit rational \textit{over-calculation}, approximating the mixed equilibrium so precisely that they under-volunteer relative to boundedly rational humans who use heuristics like "I should help."
...
Our metrics—TRS, Alignment Gap, Coalition Entropy—provide quantitative operationalizations for Dafoe et al.'s \citep{dafoe2020open} Cooperative AI agenda. Specifically:

\begin{itemize}
    \item \textbf{For robustness}: TRS$>0$ should become a standard evaluation criterion, measured by injecting realistic error rates (10-20\% based on production ML systems).
    \item \textbf{For fairness}: $|\Delta_i| < 1.0$ could serve as a "non-exploitation" constraint, preventing both Toxic Kindness and predatory value extraction.
    \item \textbf{For stability}: Coalition Entropy $H(S)$ should be minimized to encourage convergent social structures rather than chaotic oscillations.
\end{itemize}

\subsection{Limitations and Future Work}

Our study focuses on fixed personality assignments and 50-round horizons. Future work should explore:
\begin{itemize}
    \item \textbf{Adaptive personalities}: Allow agents to shift strategies based on meta-learned opponent models
    \item \textbf{N$>$3 players}: Scale to coalitions where $k$-way alliances form ($k < N$)
    \item \textbf{Communication protocols}: Test whether cheap talk or commitments mitigate exploitation
    \item \textbf{Human-AI mixed systems}: Validate findings in human-LLM hybrid games
    \item \textbf{Adversarial robustness}: Test against agents explicitly trained to exploit Toxic Kindness
\end{itemize}
...
Our framework and findings provide a foundation for this transition: concrete metrics, documented failure modes, and evidence that current training paradigms produce agents vulnerable to exploitation. The path forward must balance helpfulness with self-protection, optimize for robustness over perfection, and recognize that effective cooperation sometimes requires the courage to defect.
