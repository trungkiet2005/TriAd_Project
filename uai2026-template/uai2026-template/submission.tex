...
\textbf{Computational Resources:} All experiments were conducted on local GPU infrastructure using vLLM for efficient inference, consuming approximately 800 GPU hours on 4$\times$A100-80GB GPUs. Smaller models (7--8B parameters) achieved ~150 tokens/sec, while larger models (70B) operated at ~25 tokens/sec. Complete experimental configurations and model checkpoints are provided in supplementary materials for full reproducibility.
...
The results reveal a dramatic reversal: without belief tracking, every model reverts to the classical brittleness predicted by evolutionary game theory \citep{nowak1992tit}. The average TRS plummets by 0.26 points---transforming robust cooperators into fragile defectors. Llama-3.1-70B's story is particularly striking: with beliefs, it thrives under noise (TRS = +0.18); without them, it collapses into noise-induced defection spirals (TRS = -0.12). This 0.30-point swing, replicated across all models, demonstrates that the anti-fragility we observe is \textbf{not} merely verbose reasoning or longer prompts. It requires something specific: maintaining explicit, updateable models of what opponents intend versus what they accidentally execute.

The pattern holds across the noise spectrum. As we increase error rates from 0\% to 20\%, top models with belief tracking show \textit{rising} cooperation---72\% at baseline, climbing to 85\% at 10\% noise, and reaching 89\% at 20\% noise. Smaller models like Qwen-2.5-32B follow the same trajectory: 69\% $\to$ 82\% $\to$ 87\%. This isn't a one-off artifact at a single noise level; it's a fundamental shift in how agents interpret ambiguous signals. Traditional game theory says noise destroys cooperation linearly. These models, when equipped with belief tracking, flip the script entirely.
...
\begin{tabular}{lcc}
\toprule
\textbf{Strategy} & \textbf{Cooperation} & \textbf{Noise Robustness} \\
\midrule
Tit-for-Tat & High & Collapses \\
Generous TFT & High & Modest gain \\
Win-Stay-Lose-Shift & Medium & Collapses \\
\midrule
Llama-3.1-70B & Highest & Strong gain \\
Qwen-2.5-32B & High & Strong gain \\
Mistral-7B & Medium & Slight loss \\
\bottomrule
\end{tabular}
\caption{Classical strategies vs.~LLMs under noise. Traditional algorithms collapse or show modest gains when errors occur. Top LLMs with belief tracking show strong robustness gains---cooperation actually increases under moderate noise.}
\label{tab:baselines}
\end{table}

The story is striking. Tit-for-Tat---champion of Axelrod's famous tournaments \citep{axelrod1981evolution}---maintains high cooperation in perfect conditions but crumbles when noise enters the picture. A single accidental defection triggers endless retaliation spirals, exactly as evolutionary theory predicts. Generous TFT improves matters by forgiving randomly about 10\% of the time, showing modest robustness gains. But this forgiveness is blind---it forgives friend and foe alike, with no distinction between chronic exploiters and reliable partners who made one mistake.

Llama-3.1-70B and Qwen-2.5-32B take a fundamentally different approach. Instead of hardcoded forgiveness rates, they maintain running beliefs about each opponent's intentions. After 20 rounds of cooperation, a single defection gets interpreted charitably (``probably a glitch''). But chronic defectors received increasingly harsh treatment. This adaptive, context-sensitive forgiveness explains why top LLMs exceed even Generous TFT's performance while showing stronger robustness to noise.
